...
bool HttpDashelTarget::sendEvent(const AeslProgram& program, const std::vector<std::string>& args)
{
	if(args.empty()) {
		return false;
	}
	...
	if(program.getCommonDefinitions().events.contains(UTF8ToWString(args[0]), &eventPos)) {
		try {
...
bool HttpDashelTarget::compileAndRunCode(unsigned globalNodeId, const AeslProgram& program, const std::string& code, std::string& errorString)
{
	map<unsigned, Node>::iterator query = nodes.find(globalNodeId);
	if(query == nodes.end()) {
		if(interface->isVerbose()) {
	...
	compiler.setCommonDefinitions(&program.getCommonDefinitions());
